...
function EditProfileInner(
  { reel, onSaveReelProfile, onToast, onDirtyChange },
  ref,
) {
  ...
  useEffect(() => {
    ...
    setItem([
      ...
      { label: "한줄소개", value: reel.motto ?? "", placeholder: "한 줄 소개" },
    ]);
    setHasChange(false);
    onDirtyChange?.(false);
  }, [reel]);
  ...
  // 부모에게 더티 여부 통지
  useEffect(() => {
    onDirtyChange?.(hasChange);
  }, [hasChange, onDirtyChange]);
  ...
  // 외부에서 호출할 수 있는 저장/더티 API
  useImperativeHandle(ref, () => ({
    hasUnsaved: () => hasChange,
    save: async () => {
      console.log("profile saved by saveall")
      const data = {
        name: item[0].value,
      ...
      await onSaveReelProfile({ img, item: data });
      setHasChange(false);
      onDirtyChange?.(false);
      ...
      <div className="flex flex-col gap-10 md:h-[400px] md:flex-row md:items-start">
        ...
        >
          {img !== null ? (
            ...
            <div className="bg-black-200 absolute inset-0 flex h-full w-full items-center justify-center rounded-2xl border border-dashed text-center">
              <IoImageSharp size={100} color="white" opacity={0.2} />
            </div>
          )}
          <div className="absolute inset-0 hidden items-center justify-center bg-black/50 transition-opacity duration-200 md:flex md:opacity-0 md:group-hover:opacity-100">
        ...
        <motion.div variants={fadeUp}>
          ...
          <button
            onClick={async () => {
              await (ref?.current?.save?.() ?? Promise.resolve());
            }}
            className={`pointer-events-auto my-4 w-full rounded-lg border py-2 text-center text-base ${hasChange ? "bg-white text-black" : "bg-black-300 text-white-300 border-white-300"}`}
          >
            저장
          </button>
        </motion.div>
...
export default forwardRef(EditProfileInner);
