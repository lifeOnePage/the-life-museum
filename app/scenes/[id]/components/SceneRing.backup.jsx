...
import { useVideoTexture, Line, Text } from "@react-three/drei";
...
// ===== 아이템 레이블 (세로 텍스트) =====
function ItemLabel({ position, title, date, rotation }) {
  // 제목과 날짜를 세로로 배치
...
function RingInner({
  ...
  useFrame((_, dt) => {
    ...
    // 아이템 레이블 위치 업데이트 (전시 모드일 때만)
    if (isExhibitionMode && items && itemRanges) {
      ...
      items.forEach((item) => {
        ...
        const baseAngle = newBaseAngles[startIndex];
        ...
        newLabelPositions.push({
          ...
          date: item.date || '',
          position: [x, y, z],
          rotation: [0, -worldAngle + Math.PI / 2, 0],
        });
      });
  ...
  // 전시 모드일 때는 중앙에 배치
  const groupPositionY = isExhibitionMode ? 0 : (isDesktop ? -3 : 0);
  ...
  return (
    <group position={[0, groupPositionY, 0]} rotation={[0, 0, 0]}>
      ...
      {/* 아이템 레이블 (전시 모드일 때만) */}
      {isExhibitionMode && labelPositions.map(({ itemId, title, date, position, rotation }) => (
      ...
      {/* Leftmost Sprite - 주석 처리: DOM 요소로 대체 */}
      {/* {!isExhibitionMode && (
        <LeftmostSprite
      ...
      {/* Projection Lines - 주석 처리: DOM 요소로 대체 */}
      {/* {!isExhibitionMode && (
        <ProjectionLines
